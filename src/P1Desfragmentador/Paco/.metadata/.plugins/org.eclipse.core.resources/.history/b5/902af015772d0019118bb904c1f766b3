...
public final class Ventana1 extends JFrame implements ActionListener {
	...
	class cola_de_Digitos implements Serializable {
		...
		private int n_digitos = na.nextInt(((n*m)/5))+2;//Este valor debe de ser aleatorio por cada color entre 2 a (#total de casillas/#de colores) que es el numero de fragmentos que tendra un color
		/**
